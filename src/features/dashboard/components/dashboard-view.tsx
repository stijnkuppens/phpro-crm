  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard title="Open Deal Waarde" value={formatEUR(stats.openDealValue)} icon={DollarSign} />
        <StatCard title="Accounts" value={String(stats.totalAccounts)} icon={Building2} />
        <StatCard title="Activiteiten (7d)" value={String(stats.upcomingActivities)} icon={Calendar} />
      </div>
            ...
            <div className="space-y-3">
              {recentActivities.map((a) => (
                <div key={a.id} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">{new Date(a.date).toLocaleDateString('nl-BE')}</span>
                </div>
              ))}
            </div>
          )}
